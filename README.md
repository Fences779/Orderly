...
## 文件状态
- `docs/CLOUD_SYNC_V1_END_TO_END_CLOSURE_CHECKLIST.md`、`README.md`（新增/已修改）：新增 Cloud Sync v1 真正上线可用的总收口标准和现有代码对照清单。文档按 G1-G9、性能稳定、客户端兼容和最终验收证据逐项列出必须达成的标准，并基于当前 `Orderly.Server`、DbUp、SignalR、权限、设备、同步、冲突、附件、备份、ECS runbook 和本地验收报告标注已具备、部分具备和未具备；当前结论为工程基础较完整但生产端到端未收口，正式域名/HTTPS、T10-C、真实多设备、全实体冲突矩阵、生产恢复演练和 72 小时稳定观察仍未完成。
- `THREAD10_ECS_DEPLOYMENT_REPORT.md`、`deploy/ecs/docker-compose.prod.yml`、`deploy/ecs/.env.prod.example`、`deploy/ecs/T10_DEPLOYMENT_RUNBOOK.md`、`deploy/ecs/MIGRATION_RUNBOOK.md`、`deploy/ecs/BACKUP_RESTORE_RUNBOOK.md`、`deploy/ecs/README.md`、`deploy/ecs/SECRETS_CHECKLIST.md`、`scripts/ecs-preflight.ps1`、`src/Orderly.Server/Program.cs`（已修改）：Thread 10 T10-B adoption plan 与 T10-C1 pre-deployment gate 完成。现有 `/opt/orderly`、`orderly-postgres`、`orderly-redis`、`compose_postgres_data`、`compose_redis_data`、`compose_default` 作为 known existing Orderly baseline 保留；新生产 compose 为独立 `orderly-app` project，只管理 API/Caddy，通过 external network 访问现有 PostgreSQL/Redis，不声明旧 volume，不重建旧容器；API 支持 `ORDERLY_POSTGRES_PASSWORD_FILE`，并以 `ORDERLY_APP_UID/GID=1000:1000` 读取既有 secret。T10-C1 已完成远端 preflight、schema/journal 审计、`pg_dump -Fc` 迁移前备份、`pg_restore -l` 格式验证、镜像暂存和远端 compose config；当前 DB 为空库，待执行 DbUp 0001-0007，无 DROP/DELETE/TRUNCATE，备份文件为 `/opt/orderly/backups/orderly_pre_migration_t10c1_20260709T164535Z.dump`。T10-C1 结论 PASS，允许建议进入 T10-C2 启动 API 并执行 migration，但不允许进入 T11。
- `THREAD9_ECS_PREP_REPORT.md`、`deploy/ecs/docker-compose.prod.yml`、`deploy/ecs/.env.prod.example`、`deploy/ecs/Caddyfile.example`、`deploy/ecs/README.md`、`deploy/ecs/SECRETS_CHECKLIST.md`、`deploy/ecs/MIGRATION_RUNBOOK.md`、`deploy/ecs/BACKUP_RESTORE_RUNBOOK.md`、`deploy/ecs/T10_DEPLOYMENT_RUNBOOK.md`、`scripts/ecs-preflight.ps1`（新增）：Thread 9 ECS 部署前准备完成。新增生产 Compose 模板、生产 env 占位模板、Caddy 代理模板、secret 清单、migration 策略、备份恢复策略、T10 人工部署 runbook 和只读预检脚本；PostgreSQL/API 不暴露公网端口，API 仅经 reverse proxy 对外，模板无真实 secret，T9 不执行 ECS 部署、不 SSH、不连接生产库、不配置真实域名/HTTPS；预检脚本和 compose config 通过，T9 结论 PASS，允许进入 T10 人工实际部署流程。
- `THREAD8_DOCKER_COMPOSE_REPORT.md`、`docs/local-docker-compose-runbook.md`、`scripts/run-local-docker-compose-smoke.ps1`、`tests/Orderly.Tests/Server/Integration/ComposeSmokeTests.cs`、`.env.example`、`docker-compose.yml`、`Dockerfile`、`src/Orderly.Server/Services/LocalFileBlobStorage.cs`、`src/Orderly.Server/Program.cs`、`src/Orderly.Server/Models/ServerOptions.cs`、`.gitignore`（新增/已修改）：Thread 8 本地 Docker Compose 验收完成。新增本地 Compose 专用 env 模板、容器外 API smoke、空库 migration 检查、API/Sync/权限/设备/审计/附件/永久删除/重启恢复验收脚本；默认只启动 PostgreSQL + API，Caddy 仅保留为可选 `edge` profile；本地附件使用容器 volume 文件存储，不连接生产 OSS；T8 结论 PASS，允许进入 ECS 部署准备，不允许直接实际部署。
- `THREAD7_INTEGRATION_REPORT.md`、`docs/local-integration-runbook.md`、`scripts/run-local-integration-smoke.ps1`、`tests/Orderly.Tests/Server/Integration/LocalIntegrationSmokeTests.cs`、`src/Orderly.Server/Controllers/AdminController.cs`、`UsersController.cs`、`src/Orderly.Server/Services/CloudAuthService.cs`、`ICloudAuthService.cs`、`tests/Orderly.Tests/Server/Integration/ServerWebApplicationFactory.cs`（新增/已修改）：Thread 7 本地一体化验收入口补齐。新增 T7 smoke 测试覆盖用户申请/审批、设备批准/拒绝/撤销、权限快照、越权拒绝、Sync A/B/C、字段级冲突、幂等、Cursor、SignalR 通知边界、附件授权、永久删除、Admin/Ops health/backup/sync endpoints；补设备拒绝接口，补成员授权审计，修复 Admin 审计日志空时间筛选 500；新增本地运行脚本和 runbook。
- `THREAD6_FIX_REPORT.md`、`THREAD1_HANDOFF.md`、`THREAD2_HANDOFF.md`、`THREAD3_HANDOFF.md`、`THREAD4_HANDOFF.md`、`docs/cloud-sync-*.md`、`src/Orderly.Contracts/*`、`src/Orderly.Server/*`、`deploy/orderly.env.example`（新增/已修改）：Thread 6 Integration Fixer 只收口 `docs/THREAD5_INTEGRATION_AUDIT.md` 中的 Blocker / High Risk。补齐 Cloud Sync v1 协议入口和 handoff 基线；写入契约补 `Version/BaseVersion/ChangedFields/IdempotencyKey`；DbUp 迁移改为逐脚本事务并补审计字段；Snapshot token 改为服务端 HMAC 签名；JWT 和 Bootstrap Admin 不再静默降级到固定弱配置；业务写入、成本字段、历史版本、附件访问、导出、用户/设备/申请等入口补服务端权限、审计或幂等收口；应急草稿补 `SubmittedByUserId` 插入和草稿 Id 幂等。
- `src/Orderly.Server/Program.cs`、`src/Orderly.Server/Controllers/ArchiveController.cs`、`src/Orderly.Server/Services/CommerceCommandService.Archive.cs`、`AliyunOssBlobStorage.cs`、`CommerceReadController.cs`、`CommerceCommandService.cs`、`ConflictExceptionMiddleware.cs`、`IdempotencyService.cs`、`CommerceCommandService.PriceChanges.Review.cs`、`tests/Orderly.Tests/Server/Integration/ArchiveIntegrationTests.cs`、`CloudAuthIntegrationTests.cs`、`PriceChangeIntegrationTests.cs`、`tests/Orderly.Tests/Ui/ProductsViewContractTests.cs`、`CashflowViewContractTests.cs`、`CustomersViewContractTests.cs`、`OrdersViewContractTests.cs`、`docs/Cloud Sync v1.txt`、`README.md`（已修改）：全量云端接入检查修复测试门禁。服务端 CORS 默认来源不再把通配符与凭证模式组合导致测试宿主/默认启动失败，未显式配置来源时回落到 `ORDERLY_PUBLIC_URL` 的站点来源；JWT Bearer 保留原始 `sub`，避免云端 API 把有效 token 拒成 401；归档恢复接口改用具体 `RecoverCommand`，不再让 JSON 反序列化抽象命令基类；服务层权限拒绝统一返回 403；幂等重复请求不再通过唯一键异常打坏事务，归档等抽象实体结果会按运行时具体 DTO 保存并按资源类型还原；改价审批的商品变更和申请状态变更使用独立同步序号，避免云端变更日志主键冲突；OSS、现金流摘要和命令幂等入口补齐空值边界；集成测试按真实 scoped 生命周期解析云认证服务；UI 静态契约测试补齐已存在的归档、改价申请、权限显示绑定；Cloud Sync 文档清理受限词，保证禁用词回归能通过。
- `src/Orderly.Server/Controllers/AdminController.cs`、`README.md`（新增/已修改）：Cloud Sync v1 管理后台补上受控后端入口。云端管理员可通过工作区管理 API 查看服务/数据库/同步/备份健康状态、备份与恢复演练状态、按动作/实体/时间筛选审计日志，并查看待处理或失败的同步草稿、近期登录拒绝和同步序号状态；普通成员不能访问这些管理接口。
- `src/Orderly.Contracts/Commerce/CloudAttachmentDto.cs`、`CloudEntityVersionDto.cs`、`PermanentDeleteRequest.cs`、`src/Orderly.Server/Migrations/0006_LifecycleAttachmentsAndHistory.sql`、`src/Orderly.Server/Services/CloudDataLifecycleService.cs`、`ICloudDataLifecycleService.cs`、`src/Orderly.Server/Controllers/LifecycleController.cs`、`src/Orderly.Server/Services/CommerceCommandService.cs`、`src/Orderly.Server/Models/ServerOptions.cs`、`src/Orderly.Server/Program.cs`（新增/已修改）：Cloud Sync v1 数据生命周期补上服务端基础能力。云端写命令提交后会记录实体历史版本快照；新增历史版本查询、附件上传/下载/归档接口，附件元数据保存归属、哈希、版本、大小并按 Workspace 配额控制；附件下载必须走已登录工作区授权；永久删除只允许云端管理员确认，并要求数据已归档且达到保留期，同时写审计和同步变更记录。
- `src/Orderly.Contracts/Offline/CloudOutboxEntryDto.cs`、`CloudOutboxStatus.cs`、`ICloudOutboxStore.cs`、`src/Orderly.Data/Cloud/CloudOutboxStore.cs`、`src/Orderly.Data/Sqlite/CloudCacheSchemaInitializer.cs`、`tests/Orderly.Tests/Cloud/CloudOutboxStoreTests.cs`（新增/已修改）：Cloud Sync v1 同步闭环补上客户端本地 Outbox 基础设施。待上传变更现在有独立持久表，按 `ClientRequestId` 去重，记录实体、操作、payload、基础版本、失败原因、重试次数和下次重试时间；应用异常退出或重启后仍能取回待提交变更，失败项只会在到达重试时间后重新进入待上传队列。
- `src/Orderly.Contracts/Auth/CloudInvitation*.cs`、`CloudUserApplication*.cs`、`CloudDevice*.cs`、`CreateInvitationRequest.cs`、`SubmitUserApplicationRequest.cs`、`ReviewUserApplicationRequest.cs`、`src/Orderly.Server/Services/CloudAuthService.cs`、`ICloudAuthService.cs`、`ICloudPermissionService.cs`、`src/Orderly.Server/Controllers/AuthController.cs`、`UsersController.cs`、`src/Orderly.Server/Program.cs`、`src/Orderly.Server/Data/MigrationRunner.cs`、`src/Orderly.Server/Migrations/0005_UserApplicationsAndDevices.sql`、`src/Orderly.Remote/Clients/RemoteAuthClient.cs`、`tests/Orderly.Tests/Server/CloudPermissionServiceTests.cs`、`tests/Orderly.Tests/Server/Integration/CloudAuthIntegrationTests.cs`（新增/已修改）：Cloud Sync v1 用户与设备入口门禁完成第一段收口。新增邀请码、用户申请、云端管理员审批、首设备激活、后续设备待审批、设备批准/撤销接口；登录和 refresh token 绑定设备号，JWT 缺设备号直接拒绝，设备撤销后同步吊销该设备 refresh token；用户/邀请/设备审批入口收紧为云端管理员标签，普通用户仍可查看并撤销自己的设备；集成测试宿主改为使用测试容器环境变量，迁移 runner 改为遵守脚本内事务，启动管理员初始化改为 scoped 解析。
