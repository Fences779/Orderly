...
## 文件状态
- `src/Orderly.Contracts`、`src/Orderly.Server`、`src/Orderly.Remote`、`Orderly.sln`、`src/Orderly.App/App.WorkspaceComposition.cs`、`src/Orderly.App/App.xaml.cs`、`src/Orderly.App/ViewModels/MainViewModel.CommercePages.cs`、`README.md`（新增/已修改）：按团队云端正式版方案完成阶段 1/2/3/4 云端骨架与数据链路，新增 Contracts 共享 DTO/命令/事件/权限，ASP.NET Core 服务端（PostgreSQL + DbUp 迁移 + JWT/Refresh/账号权限/审计 + Commerce 读写查询与员工脱敏 + 事务内幂等/版本冲突/审计/变更日志 + SignalR Hub + 健康检查），Remote HTTP/SignalR 客户端、服务与仓储适配器；WPF 组合根支持 `ORDERLY_RUNTIME=Cloud|LocalDev` 分支，SignalR 云端事件已按实体类型细化到工作台/订单/商品/库存/客户/现金流/经营建议页面失效，避免全量刷新；远程端已实现订单完成、库存变动、现金流记录与结算、客户/订单/现金流的创建/更新/归档，库存/商品主数据暂仅支持归档。导出、同步/补拉、云登录流程、SQLCipher 本地缓存、OSS 备份脚本待后续阶段完成。
