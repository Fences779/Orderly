...
## 文件状态
- `src/Orderly.Contracts`、`src/Orderly.Server`、`src/Orderly.Remote`、`Orderly.sln`、`src/Orderly.App/App.WorkspaceComposition.cs`、`src/Orderly.App/App.xaml.cs`、`src/Orderly.App/ViewModels/MainViewModel.CommercePages.cs`、`README.md`（新增/已修改）：按团队云端正式版方案完成阶段 1/2 骨架与只读云端数据链路，新增 Contracts 共享 DTO/命令/事件/权限、ASP.NET Core 服务端（PostgreSQL + DbUp 迁移 + JWT/Refresh/账号权限/审计 + Commerce 只读查询与员工脱敏 + SignalR Hub + 健康检查）、Remote HTTP/SignalR 客户端与服务适配器、本地缓存/应急草稿存储；WPF 组合根支持 `ORDERLY_RUNTIME=Cloud|LocalDev` 分支与云端事件触发的页面刷新标记；写操作、归档/恢复、改价、导出、同步/补拉等接口已搭好 Controller 骨架，待后续阶段填充业务事务。
