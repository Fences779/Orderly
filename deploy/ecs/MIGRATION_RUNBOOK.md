...
- PostgreSQL 是现有 external dependency：`orderly-postgres` + `compose_postgres_data`。
- 不由新应用 compose 创建、删除、重建或改名 PostgreSQL。
- 不重写历史 migration。
- 不清空生产库。
- 不跳过失败 migration。
...
T10-A 已确认现有 `orderly` 数据库存在，且未发现业务表。T10-C 的“首次接管”不是创建数据库，而是让新 API 连接现有 `orderly-postgres` 并执行 DbUp。

1. 确认 T10-B PASS。
2. 只读确认 `orderly-postgres` running、`compose_postgres_data` 已挂载。
3. 只读确认 `/opt/orderly/env/postgres_password` 存在且权限合适，不输出内容。
4. 写入真实 `/opt/orderly/env/orderly.prod.env`，确认 `ASPNETCORE_ENVIRONMENT=Production`。
5. 执行 `docker compose config --quiet`，确认 services 不包含 `postgres` 或 `redis`。
6. 首次启动 API：
...
- 先停新应用容器，确认旧 `orderly-postgres` 和旧 volume 未被改动。
...
- 禁止重建 `orderly-postgres`。
- 禁止重建 `compose_postgres_data`。
- 禁止跳过 DbUp 记录直接改表。
- 禁止使用本地测试 `.env` 连接生产库。
