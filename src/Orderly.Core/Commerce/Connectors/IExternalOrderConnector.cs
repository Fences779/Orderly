...
/// <summary>
/// Reserved, neutral connector specialization for exchanging order data with an external system
...
/// </para>
/// </summary>
public interface IExternalOrderConnector : IExternalConnector
{
    /// <summary>
    /// Pushes a batch of order records to the external system. Disabled in V1: returns a
    /// <see cref="ConnectorOutcome.ConnectorDisabled"/> result with no outbound request.
    /// </summary>
    /// <param name="records">The neutral order records to push.</param>
    /// <param name="cancellationToken">A cancellation token.</param>
    Task<ConnectorResult> PushOrdersAsync(
        IReadOnlyList<ConnectorRecord> records,
        CancellationToken cancellationToken = default);

    /// <summary>
    /// Pulls order records from the external system. Disabled in V1: returns a
    ...
    /// <param name="cancellationToken">A cancellation token.</param>
    Task<ConnectorResult<IReadOnlyList<ConnectorRecord>>> PullOrdersAsync(
